...
from . import main
from .forms import EditProfileAdminForm,PostForm,EditProfileForm, CommentForm
from .. import db
...
def server_shutdown():
    if not current_app.testing:
        abort(404)
    shutdown = request.environ.get('werzeug.server.shutdown')
    if not shutdown:
        abort(500)
    shutdown()
...
def followed_by(username):
    ...
    if user is None:
        flash('Invalid user.')
        return redirect(url_for('.index'))
    page = request.args.get('page',1,type=index)
